...
(function () {
  ...
  function drawFlywheel() {
    ...
    stageInfo = bounds.map((b, i) => {
      const am = start + i * seg + seg / 2, bp = pt(am, rMid + 14); // same spot as the slab's own badge, so the glow disc covers it instead of doubling it
      return { d: b.d, c: col[i], ix: bp.x, iy: bp.y, ic: STAGES[i].ic, k: STAGES[i].k };
    });
    if (!glowEl) {
...
(function () {
  ...
  document.addEventListener('click', function (e) { if (tip.contains(e.target)) return; const el = e.target.closest && e.target.closest('[data-tip-name]'); if (el) { if (pinned && el === curEl && tip.classList.contains('show')) { hideNow(); e.stopPropagation(); return; } pinned = false; open(el); pinned = true; e.stopPropagation(); } else hideNow(); });
})();
